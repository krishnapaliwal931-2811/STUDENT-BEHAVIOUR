...
1.  *Cleaning Up:* We first tidied the data. This involved removing about 91 entries where we were missing the student's parental education level to ensure our model only trained on complete and reliable information.
...
3.  *Testing the Models:* We tested two popular predictive models:
...
4.  *Selecting the Best Model:* We used a rigorous testing method (Cross-Validation) to see which model performed best consistently.
...
In practical terms, the model can predict a student's exam score with an average error of about *3.47 points* (the RMSE).
